...
curl_download(
  URLencode('https://www.ebi.ac.uk/ena/portal/api/search?result=assembly&query=tax_tree(6157) AND genome_representation="full"&fields=version,tax_id,scientific_name,last_updated,base_count&limit=0'),
              destfile = 'z1.txt')
```
En este primero faltaba "URLencode"
```{r}
curl_download(
  URLencode('https://www.ebi.ac.uk/ena/portal/api/search?result=assembly&query=tax_tree(6157) AND genome_representation="full"&fields=version,tax_id,scientific_name,last_updated,base_count&limit=0'),
  destfile = 'z1.txt')
...
```{r}
curl_download(URLencode('https://www.ebi.ac.uk/ena/portal/api/search?result=sequence&query=tax_tree(32281) AND description="ADH"&fields=accession,description,tax_id,scientific_name,base_count,first_public&limit=0'), destfile = 'z3.2.txt')
```

```{r}
adh <- read.table('z3.2.txt', sep = '\t',
                  quote= "", header = TRUE)
```

1. ¿Cuántas secuencias hay con el término "alcohol dehydrogenase" en su descripción? Aparecen 558 secuencias.
2. ¿Qué longitudes tienen? De media contienen 846
> summary(adh$base_count)
   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
  109.0   711.0   846.0   891.7   884.0 13087.0 
  
3. ¿De cuántas especies diferentes son estas secuencias? DE 136 especies diferentes

4. ¿Cuándo se publicaron las más recientes? El 14 de junio del 2023.
...
Deberíamos agrupar la longitud por fecha y no por genero. 
```{r}
#| fig.width: 10
par(mar = c(8,7,1,1))
