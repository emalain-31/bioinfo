...
20 primeros registros de tipo Pfam (esta es la fuente de los datos) y exploramos el resultado.
...
```{r}
library('kableExtra')
pfam02 <- fromJSON(pfam01[['next']]) #asi estas creando una nueva URL con las 20 siguientes entradas
kable(pfam02[['results']][['metadata']]) %>% kable_styling() #kable sirve para crear tablas
```
...
str(pf06235)
nombre_largo <- pf06235[['metadata']][['name']][['name']] #Has definido ese elemento que es NADH deshidrogenase... como nombre largo. Entonces cuando escribas nombre_largo te dara ese elemento.fr
nombre_largo
```
...
- https://www.ebi.ac.uk/interpro/api/protein/entry (esta URL te va a dar el numeor de proteinas que contienen entrys)
...
https://www.ebi.ac.uk/interpro/api/taxonomy -> 2678841
https://www.ebi.ac.uk/interpro/api/taxonomy/protein -> 1291419

2678841 - 1291419 = 1387422 taxones de UniProtKB no tienen ninguna proteína asignada
...
https://www.ebi.ac.uk/interpro/api/protein -> Reviewed:571702  Unreviewed: 201794580   Uniprot: 202150435
https://www.ebi.ac.uk/interpro/api/protein/structure -> Reviewed:37334  Unreviewed: 30128   Uniprot: 67874

Reviewed:6,53%  Unreviewed: 0,015%   Uniprot: 0,033%
:::
...
```{r}
t7742 <- fromJSON(paste0(API, 'entry/pfam/taxonomy/uniprot/7742'))
names(t7742)
t7742[['count']]
names(t7742[['results']])

# La función "flaten()" corrige el anidamiento de marcos de datos,
kable(flatten(t7742[['results']][['metadata']])) %>%
   kable_styling()
```
...
```{r}
cocodrilo <- fromJSON(paste0(API, 'protein/entry/pfam/PF01530/taxonomy/uniprot/8493')) #lo primero que pones es lo que buscas, en este caso proteinas, y luego pones los filtros
names (cocodrilo)
cocodrilo[['proteins']]
```
